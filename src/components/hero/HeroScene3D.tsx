...
// Gift box component with hover tilt
function GiftBox({
  position,
  size = 1,
  color,
  ...
  useFrame((state) => {
    ...
    // Subtle pointer-based tilt on desktop
  ...
  return (
    <Float speed={speed * 1.5} floatIntensity={floatIntensity} rotationIntensity={0.2}>
...
function Sparkle({ position }: { position: [number, number, number] }) {
  ...
  useFrame((state) => {
    ...
    ref.current.material && ((ref.current.material as THREE.MeshStandardMaterial).opacity =
...
// Scroll-based parallax layer offset
...
function Scene() {
  ...
  return (
    <>
      ...
      <ParallaxGroup>
        {/* Main gift boxes at varying depths */}
